...
describe('zkLogin address', () => {
	...
	test('computeZkLoginAddressFromSeed matches ZkLoginPublicIdentifier.toHExAddress() for legacy addresses', () => {
		const seed = BigInt(
		...
		expect(computeZkLoginAddressFromSeed(seed, iss)).toEqual(
			toZkLoginPublicIdentifier(seed, iss, { legacyAddress: true }).toHexAddress(),
		);
	});

	test('computeZkLoginAddressFromSeed matches ZkLoginPublicIdentifier.toHexAddress() for non-legacy addresses', () => {
		const seed = BigInt(
			'380704556853533152350240698167704405529973457670972223618755249929828551006',
		);
		const iss = 'https://accounts.google.com';
		expect(computeZkLoginAddressFromSeed(seed, iss, false)).toEqual(
			toZkLoginPublicIdentifier(seed, iss).toHexAddress(),
		);
	});
	...
	test('a valid JWT should not throw an error', () => {
		...
		expect(address).toBe('0x8fe6abe082c33c86ddf84bf7e5b2fcc7d0f3507532e05c8091c714596ec4f014');
	});
