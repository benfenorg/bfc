...
describe('Multisig scenarios', () => {
	it('multisig address creation and combine sigs using Secp256r1Keypair', async () => {
		...
		tx.setSender(k3.getPublicKey().toHexAddress());
		tx.setGasPrice(5);
		tx.setGasBudget(100);
		tx.setGasPayment([
			{
...
describe('MultisigKeypair', () => {
	test('signTransaction', async () => {
		...
		expect(signerPubKey.toHexAddress()).toEqual(publicKey.toHexAddress());
		...
		expect(signerPubKey2.toHexAddress()).toEqual(publicKey.toHexAddress());
	...
	test('unknown signers', async () => {
		...
		expect(() => publicKey.getSigner(k2)).toThrow(
			new Error(`Signer ${pk2.toHexAddress()} is not part of the MultiSig public key`),
		);
	});
});
