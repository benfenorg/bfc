...
import type { BenfenReportTransactionEffectsInput } from '../../../wallet-standard/index.js';
...
type UseReportTransactionEffectsArgs = Omit<
	PartialBy<BenfenReportTransactionEffectsInput, 'account' | 'chain'>,
	'effects'
> & {
	...
	return useMutation({
		...
		mutationFn: async ({ effects, chain = currentWallet?.chains[0], account = currentAccount }) => {
			...
			const reportTransactionEffectsFeature =
				currentWallet.features['bfc:reportTransactionEffects'];
