...
import type { BenfenClient } from '../client/client.js';
import { BFC_TYPE_ARG, normalizeBenfenObjectId, normalizeHexAddress } from '../utils/index.js';
...
async function resolveObjectReferences(
	...
	const dedupedIds = [
		...new Set(
			objectsToResolve.map((input) => normalizeBenfenObjectId(input.UnresolvedObject.objectId)),
		),
	];
	...
	for (const [index, input] of transactionData.inputs.entries()) {
		...
		const id = normalizeHexAddress(input.UnresolvedObject.objectId);
...
export function getClient(options: BuildTransactionOptions): BenfenClient {
	if (!options.client) {
		throw new Error(
			`No benfen client passed to Transaction#build, but transaction data was not sufficient to build offline.`,
		);
	}
