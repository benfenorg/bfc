...
import { normalizeHexAddress, normalizeStructTag } from '../../utils/bf-types.js';
...
it('can serialize simplified programmable call struct', () => {
	...
	expect(normalizeHexAddress(result.package)).toEqual(normalizeHexAddress(moveCall.package));
	expect(result.typeArguments[0]).toEqual(moveCall.typeArguments[0]);
});

function ref(): { objectId: string; version: string; digest: string } {
	return {
		objectId: normalizeHexAddress((Math.random() * 100000).toFixed(0).padEnd(64, '0')),
...
it('can serialize transaction data with a programmable transaction', () => {
	let bf = normalizeHexAddress('0x2');
	let txData = {
		$kind: 'V1',
		V1: {
			sender: normalizeHexAddress('0xBAD'),
