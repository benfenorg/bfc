...
export function isTxContext(param: OpenMoveTypeSignature): boolean {
	...
	return (
		!!struct &&
		normalizeHexAddress(struct.package) === normalizeHexAddress('0x2') &&
...
export function getPureBcsSchema(typeSignature: OpenMoveTypeSignatureBody): BcsType<any> | null {
	...
	if ('datatype' in typeSignature) {
		const pkg = normalizeHexAddress(typeSignature.datatype.package);

		if (pkg === normalizeHexAddress(MOVE_STDLIB_ADDRESS)) {
			if (
		...
		if (
			pkg === normalizeHexAddress(BENFEN_FRAMEWORK_ADDRESS) &&
