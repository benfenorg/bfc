...
import { bcs, toBase64 } from '../../bcs/index.js';
import type { BenfenObjectRef } from '../../bcs/types.js';
import type {
	BenfenClient,
...
export class ParallelTransactionExecutor {
	...
	#defaultGasBudget: bigint;
	#maxPoolSize: number;
	#sourceCoins: Map<string, BenfenObjectRef | null> | null;
	...
	async executeTransaction(
		...
		const { promise, resolve, reject } = promiseWithResolvers<{
			digest: string;
			effects: string;
			data: BenfenTransactionBlockResponse;
		}>();
	...
	async #execute(
		...
		try {
			transaction.setSenderIfNotSet(this.#signer.toHexAddress());
			...
			if (gasCoin && gasUsed && gasResult.owner === this.#signer.toHexAddress()) {
				const totalUsed =
	...
	async #getGasPrice(): Promise<bigint> {
		...
		const state = await this.#client.getLatestBenfenSystemState();
	...
	async #refillCoinPool() {
		...
		const address = this.#signer.toHexAddress();
