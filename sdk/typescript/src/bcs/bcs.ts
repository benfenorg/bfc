...
import { bcs, fromBase58, fromBase64, fromHex, toBase58, toBase64, toHex } from './src/index.js';
...
export const Address = bcs.bytes(BENFEN_ADDRESS_LENGTH).transform({
	validate: (val) => {
		const address = typeof val === 'string' ? val : toHex(val);
		if (!address || !isValidBenfenAddress(normalizeHexAddress(address))) {
			throw new Error(`Invalid Benfen address ${address}`);
		}
	},
	input: (val: string | Uint8Array) =>
		typeof val === 'string' ? fromHex(normalizeHexAddress(val)) : val,
	output: (val) => normalizeHexAddress(toHex(val)),
});
...
export const BenfenObjectRef = bcs.struct('BenfenObjectRef', {
	objectId: Address,
	version: bcs.u64(),
	digest: ObjectDigest,
});
...
export const ObjectArg = bcs.enum('ObjectArg', {
	ImmOrOwnedObject: BenfenObjectRef,
