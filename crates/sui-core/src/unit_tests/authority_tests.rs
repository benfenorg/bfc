...
use sui_types::messages_consensus::{AuthorityCapabilitiesV2, ConsensusCommitPrologue, ConsensusDeterminedVersionAssignments};
...
async fn prepare_authority_and_shared_stable_object_cert() -> (Arc<AuthorityState>, VerifiedCertificate, ObjectID) {
    let (sender, keypair): (_, AccountKeyPair) = get_key_pair();
...
async fn test_stable_dry_run_no_gas_big_transfer() {
    ...
    let gas_object_id = ObjectID::random();
    let (_, fullnode, _) =
        init_state_with_stable_ids_and_object_basics_with_fullnode(vec![(sender, gas_object_id)]).await;
...
async fn test_stable_dev_inspect_unowned_object() {
    ...
    let alice_gas_id = ObjectID::random();
    let (validator, fullnode, object_basics) =
        init_state_with_stable_ids_and_object_basics_with_fullnode(vec![(alice, alice_gas_id)]).await;
    let (bob, _bob_key): (_, AccountKeyPair) = get_key_pair();
...
async fn test_stable_dev_inspect_dynamic_field() {
    let (test_object1_bytes, test_object2_bytes) = {
        ...
        let gas_object_id = ObjectID::random();
        let (validator, full_node, object_basics) =
            init_state_with_stable_ids_and_object_basics_with_fullnode(vec![(sender, gas_object_id)])
                .await;
        macro_rules! mk_obj {
            () => {{
                let effects = call_move_(
...
async fn test_stable_dev_inspect_gas_price() {
    ...
    let protocol_config = epoch_store.protocol_config();
    let error = fullnode
        .dev_inspect_transaction_block(sender, kind, Some(protocol_config.max_gas_price() + 1), None, None, None, None, None)
        .await
        .unwrap_err();
    assert!(
        matches!(
...
async fn test_stable_dev_inspect_uses_unbound_object() {
    ...
    let result = fullnode
        .dev_inspect_transaction_block(
            sender,
            kind,
            Some(fullnode.reference_gas_price_for_testing().unwrap()), None, None, None, None, None,
        )
        .await;
    let Err(err) = result else { panic!() };
    assert!(err.to_string().contains("ObjectNotFound"));
...
async fn test_stable_for_inc_201_dev_inspect() {
    ...
    let DevInspectResults { events, .. } = fullnode
        .dev_inspect_transaction_block(
            sender,
            kind,
            Some(fullnode.reference_gas_price_for_testing().unwrap() + 1000), None, None, None, None, None,
        )
        .await
        .unwrap();
...
#[cfg(test)]
pub async fn init_state_with_stable_ids_and_object_basics_with_fullnode<
    I: IntoIterator<Item=(SuiAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, Arc<AuthorityState>, ObjectRef) {
...
async fn test_stable_dev_inspect_on_validator() {
    ...
    let gas_object_id = ObjectID::random();
    let (validator, object_basics) =
        init_state_with_stable_ids_and_object_basics(vec![(sender, gas_object_id)]).await;
...
async fn test_stable_handle_transfer_transaction_bad_signature() {
    ...
    let gas_object_id = ObjectID::random();
    let authority_state =
        init_state_with_stable_ids(vec![(sender, object_id), (sender, gas_object_id)]).await;
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
...
async fn test_stable_handle_transfer_transaction_with_max_sequence_number() {
    ...
    let gas_object_id = ObjectID::random();
    let recipient = dbg_addr(2);
    let authority_state = init_state_with_stable_ids_and_versions(vec![
        (sender, object_id, SequenceNumber::MAX),
        (sender, gas_object_id, SequenceNumber::new()),
    ])
        .await;
...
async fn test_stable_handle_sponsored_transaction() {
    ...
    let gas_object_id = ObjectID::random();
    let authority_state =
        init_state_with_stable_ids(vec![(sender, object_id), (sponsor, gas_object_id)]).await;
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
    let epoch_store = authority_state.load_epoch_store_one_call_per_task();
...
async fn test_stable_immutable_gas() {
    ...
    let recipient = dbg_addr(2);
    let mut_object_id = ObjectID::random();
    let authority_state = init_state_with_stable_ids(vec![(sender, mut_object_id)]).await;
...
async fn test_stable_objected_owned_gas() {
    ...
    let recipient = dbg_addr(2);
    let parent_object_id = ObjectID::random();
    let authority_state = init_state_with_stable_ids(vec![(sender, parent_object_id)]).await;
    let epoch_store = authority_state.load_epoch_store_one_call_per_task();
    let child_object_id = ObjectID::random();
    let child_object = Object::with_object_owner_for_testing(child_object_id, parent_object_id);
    authority_state
...
async fn test_stable_publish_module_no_dependencies_ok() {
    ...
    // };
    // let gas_payment_object =
    //     Object::with_stable_id_owner_gas_for_testing(gas_payment_object_id, sender, gas_balance);
    // let gas_payment_object_ref = gas_payment_object.compute_object_reference();
    // authority.insert_genesis_object(gas_payment_object).await;
    //
    // let module = file_format::empty_module();
...
async fn test_stable_handle_move_transaction() {
    ...
    let gas_payment_object_id = ObjectID::random();
    let (authority_state, pkg_ref) =
        init_state_with_stable_ids_and_object_basics(vec![(sender, gas_payment_object_id)]).await;
...
async fn test_stable_type_argument_dependencies() {
    ...
    let gas3 = ObjectID::random();
    let (authority_state, (object_basics, _, _)) =
        init_state_with_stable_ids_and_object_basics(vec![(s1, gas1), (s2, gas2), (s3, gas3)]).await;
    let epoch_store = authority_state.load_epoch_store_one_call_per_task();
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
    let gas1 = {
        let o = authority_state.get_object(&gas1).await.unwrap();
...
async fn test_stable_handle_confirmation_transaction_receiver_equal_sender() {
    ...
    let gas_object_id = ObjectID::random();
    let authority_state =
        init_state_with_stable_ids(vec![(address, object_id), (address, gas_object_id)]).await;
    let object = authority_state
        .get_object(&object_id)
        .await
        .unwrap();
    ...
    let signed_effects = authority_state
        ...
        .await
        .unwrap();
    signed_effects.summary_for_debug().status.unwrap();
}
...
async fn test_stable_handle_confirmation_transaction_ok() {
    ...
    let new_account = authority_state
        ...
        .await
        .unwrap();
    assert_eq!(new_account.owner.get_owner_address().unwrap(), recipient);
    assert_eq!(next_sequence_number, new_account.version());
...
async fn test_stable_handle_confirmation_transaction_idempotent() {
    ...
    let signed_effects = authority_state
        ...
        .await
        .unwrap();
    assert_eq!(signed_effects.summary_for_debug().status, ExecutionStatus::Success);
    ...
    let signed_effects2 = authority_state
        ...
        .await
        .unwrap();
    assert_eq!(signed_effects2.summary_for_debug().status, ExecutionStatus::Success);
    ...
    assert_eq!(
        info.status.into_effects_for_testing().summary_for_debug().status,
        signed_effects.summary_for_debug().status
    );
}
...
async fn test_stable_move_call_insufficient_gas() {
    ...
    // let gas_object_id1 = ObjectID::random();
    // let gas_object_id2 = ObjectID::random();
    // let authority_state = init_state_with_stable_ids(vec![
    //     (sender, object_id),
    //     (sender, gas_object_id1),
    //     (recipient, gas_object_id2),
    // ])
...
async fn test_stable_account_state_ok() {
    ...
    let authority_state = init_state_with_stable_object_id(sender, object_id).await;
    authority_state
        .get_object(&object_id)
        .await
        .unwrap();
...
async fn test_stable_authority_persist() {
    ...
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let obj = Object::with_stable_id_owner_for_testing(object_id, recipient);
    ...
    // Check the object is present
    assert_eq!(obj2.id(), object_id);
    assert_eq!(obj2.owner.get_owner_address().unwrap(), recipient);
}
...
async fn test_stable_idempotent_reversed_confirmation() {
    ...
    let object = Object::with_stable_owner_for_testing(sender);
    let object_ref = object.compute_object_reference();
    let gas_object = Object::with_stable_owner_for_testing(sender);
    let gas_object_ref = gas_object.compute_object_reference();
    let authority_state = init_state_with_objects([object, gas_object]).await;
    let epoch_store = authority_state.load_epoch_store_one_call_per_task();
...
async fn test_stable_transfer_sui_no_amount() {
    ...
    let signed_effects = authority_state
        ...
        .await
        .unwrap();
    let effects = signed_effects.summary_for_debug();
    // Check that the transaction was successful, and the gas object is the only mutated object,
    // and got transferred. Also check on its version and new balance.
    assert!(effects.status.is_ok());
    // assert!(effects.mutated_excluding_gas().is_empty());
...
async fn test_stable_store_revert_transfer_sui() {
    ...
    assert_eq!(
        db.get_object(&gas_object_id).unwrap().owner,
        Owner::AddressOwner(sender),
    );
    assert_eq!(
...
async fn test_stable_store_revert_add_ofield() {
    ...
    assert!(add_effects.status.is_ok());
    assert_eq!(add_effects.created_object_count, 1);
...
async fn test_stable_gas_smashing() {
    ...
    async fn run_and_check(
        ...
        let balance = sui_types::gas::get_gas_balance(
            &state.get_object(&gas_coin_ids[0]).await.unwrap(),
        )
            .unwrap();
...
async fn test_stable_publish_transitive_dependencies_ok() {
    ...
    let (sender, key): (_, AccountKeyPair) = get_key_pair();
    let gas_id = ObjectID::random();
    let state = init_state_with_stable_ids(vec![(sender, gas_id)]).await;
    let rgp = state.reference_gas_price_for_testing().unwrap();
...
#[cfg(test)]
pub async fn init_state_with_stable_ids_and_object_basics<
    I: IntoIterator<Item=(SuiAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, ObjectRef) {
...
// TODO break this up into a cleaner set of components. It does a bit too much
// currently
async fn construct_stable_shared_object_transaction_with_sequence_number(
    initial_shared_version_override: Option<SequenceNumber>,
) -> (
    Arc<AuthorityState>,
    Arc<AuthorityState>,
...
async fn test_dry_run_gas_transfer() {
    ...
    let obj = authority_state
        ...
        .await
        .unwrap();
    fullnode.insert_objects_unsafe_for_testing_only(&[obj.clone()]).await.unwrap();
    let mut builder = ProgrammableTransactionBuilder::new();
