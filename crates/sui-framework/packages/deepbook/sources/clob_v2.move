...
module deepbook::clob_v2 {
    ...
    public(package) fun create_customized_pool<BaseAsset, QuoteAsset>(
        tick_size: u64,
        lot_size: u64,
        taker_fee_rate: u64,
        maker_rebate_rate: u64,
    ...
    public(package) fun deposit_base<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        coin: Coin<BaseAsset>,
    ...
    public(package) fun deposit_quote<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
    ...
    public(package) fun withdraw_base<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        quantity: u64,
        account_cap: &AccountCap,
        ctx: &mut TxContext
    ...
    public(package) fun withdraw_quote<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        quantity: u64,
        account_cap: &AccountCap,
        ctx: &mut TxContext
    ...
    public(package) fun batch_cancel_order<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
    ...
    public(package) fun clean_up_expired_orders<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        clock: &Clock,
        order_ids: vector<u64>,
    ...
    public(package) fun list_open_orders<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>,
        account_cap: &AccountCap
    ): vector<Order> {
        let owner = account_owner(account_cap);
    ...
    public(package) fun account_balance<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>,
        account_cap: &AccountCap
    ): (u64, u64, u64, u64) {
        let owner = account_owner(account_cap);
    ...
    public(package) fun get_market_price<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>
    ...
    public(package) fun get_level2_book_status_bid_side<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>,
        mut price_low: u64,
        mut price_high: u64,
        clock: &Clock
    ...
    /// The previous is a list of all valid prices
    /// The latter is the corresponding depth list
    public(package) fun get_level2_book_status_ask_side<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>,
        mut price_low: u64,
        mut price_high: u64,
        clock: &Clock
    ...
    public(package) fun get_order_status<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>,
        order_id: u64,
        account_cap: &AccountCap
    ): &Order {
    ...
    #[test_only]
    public(package) fun setup_test_with_tick_lot(
        taker_fee_rate: u64,
        maker_rebate_rate: u64,
    ...
    public fun setup_test_with_tick_lot_and_wrapped_pool(
        ...
        test_scenario::next_tx(scenario, sender);
        {
            let (pool, pool_owner_cap) = create_pool_with_return_<BFC, USD>(
                taker_fee_rate,
                maker_rebate_rate,
                tick_size,
                lot_size,
    ...
    #[test_only]
    public(package) fun check_tick_level(
    ...
    #[test_only]
    public(package) fun order_id_for_test(
        sequence_id: u64,
        is_bid: bool
    ): u64 {
    ...
    #[test_only]
    public(package) fun borrow_mut_custodian<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>
    ...
    #[test_only]
    public(package) fun borrow_custodian<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>
    ...
    #[test_only]
    public(package) fun test_match_bid<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        account_cap: &AccountCap,
        client_order_id: u64,
        quantity: u64,
    ...
    #[test_only]
    public(package) fun test_match_bid_with_quote_quantity<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        account_cap: &AccountCap,
        client_order_id: u64,
        quantity: u64,
    ...
    #[test_only]
    public(package) fun test_match_ask<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        account_cap: &AccountCap,
        client_order_id: u64,
        quantity: u64,
    ...
    #[test_only]
    public(package) fun test_inject_limit_order<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        client_order_id: u64,
        price: u64,
        original_quantity: u64,
    ...
    #[test_only]
    public(package) fun test_inject_limit_order_with_expiration<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        client_order_id: u64,
        price: u64,
        original_quantity: u64,
    ...
    #[test_only]
    public(package) fun get_pool_stat<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>
    ...
    #[test_only]
    public(package) fun get_usr_open_orders<BaseAsset, QuoteAsset>(
        pool: &Pool<BaseAsset, QuoteAsset>,
        owner: address
    ): &LinkedTable<u64, u64> {
        assert!(contains(&pool.usr_open_orders, owner), 0);
    ...
    #[test_only]
    public(package) fun test_construct_order(sequence_id: u64, client_order_id: u64, price: u64, original_quantity: u64, quantity: u64, is_bid: bool, owner: address): Order {
        Order {
    ...
    #[test_only]
    public(package) fun test_construct_order_with_expiration(
        sequence_id: u64,
        client_order_id: u64,
        price: u64,
        original_quantity: u64,
    ...
    #[test_only]
    public(package) fun check_usr_open_orders(
    ...
    #[test_only]
    public(package) fun test_remove_order<BaseAsset, QuoteAsset>(
        pool: &mut Pool<BaseAsset, QuoteAsset>,
        tick_index: u64,
        sequence_id: u64,
        is_bid: bool,
    ...
    fun test_place_limit_order_with_invalid_restrictions_() {
        ...
        test_scenario::next_tx(&mut test, alice);
        {
        let mut pool = test_scenario::take_shared<Pool<BFC, USD>>(&test);
        ...
        custodian::deposit(
        &mut pool.base_custodian,
        mint_for_testing<BFC>(1000 * 100000000, test_scenario::ctx(&mut test)),
        account_cap_user
        );
        custodian::deposit(
        &mut pool.quote_custodian,
    ...
    fun test_place_limit_order_with_restrictions_FILL_OR_KILL_() {
        ...
        custodian::assert_user_balance<BFC>(&pool.base_custodian, account_cap_user, 0, 1000 * 100000000);
        test_scenario::return_shared(pool);
        test_scenario::return_shared(clock);
        test_scenario::return_to_address<AccountCap>(alice, account_cap);
        };
        ...
        custodian::deposit(
        &mut pool.base_custodian,
        mint_for_testing<BFC>(900 * 100000000, test_scenario::ctx(&mut test)),
        account_cap_user
        );
        custodian::assert_user_balance<BFC>(&pool.base_custodian, account_cap_user, 900 * 100000000, 0);
        place_limit_order<BFC, USD>(
        &mut pool,
        CLIENT_ID_BOB,
        4 * FLOAT_SCALING,
    ...
    fun test_place_limit_order_with_restrictions_IMMEDIATE_OR_CANCEL() {
        ...
        let (base_filled, quote_filled, maker_injected, maker_order_id) = place_limit_order<BFC, USD>(
        &mut pool,
        CLIENT_ID_ALICE,
        10 * FLOAT_SCALING,
        1000 * 100000000,
        ...
        let (base_filled, quote_filled, maker_injected, _) = place_limit_order<BFC, USD>(
        &mut pool,
        CLIENT_ID_ALICE,
        4 * FLOAT_SCALING,
        800 * 100000000,
        ...
        custodian::assert_user_balance<BFC>(&pool.base_custodian, account_cap_user, 300 * 100000000, 0);
        {
    ...
    #[test_only]
    public(package) fun check_balance_invariants_for_account<BaseAsset, QuoteAsset>(
        account_cap: &AccountCap,
        quote_custodian: &Custodian<QuoteAsset>,
        base_custodian: &Custodian<BaseAsset>,
        pool: &Pool<BaseAsset, QuoteAsset>
