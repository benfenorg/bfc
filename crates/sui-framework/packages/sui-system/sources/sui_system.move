...
public entry fun request_add_stake_mul_coin(
    ...
    let self = load_system_state_mut(wrapper);
    let staked_sui = sui_system_state_inner::request_add_stake_mul_coin(
        self,
        stakes,
        stake_amount,
        validator_address,
...
public entry fun request_withdraw_stable_stake<STABLE>(
    ...
    let (withdrawn_stake, reward) = request_withdraw_stable_stake_non_entry(
        wrapper,
        staked_sui,
        ctx,
    );
...
public entry fun update_validator_next_epoch_protocol_pubkey(
    ...
    let self = load_system_state_mut(self);
    sui_system_state_inner::update_validator_next_epoch_protocol_pubkey(
        self,
        protocol_pubkey,
        proof_of_possession,
        ctx,
...
public entry fun update_candidate_validator_protocol_pubkey(
    ...
    let self = load_system_state_mut(self);
    sui_system_state_inner::update_candidate_validator_protocol_pubkey(
        self,
        protocol_pubkey,
        proof_of_possession,
        ctx,
...
// 3. Distribute computation charge to validator stake.
// 4. Update all validators.
#[allow(unused_function), test_only]
/// This function should be called at the end of an epoch, and advances the system to the next epoch.
/// It does the following things:
/// 1. Add storage charge to the storage fund.
/// 2. Burn the storage rebates from the storage fund. These are already refunded to transaction sender's
...
/// 3. Distribute computation charge to validator stake.
/// 4. Update all validators.
fun advance_epoch_with_stable_rate(
    storage_reward: Balance<BFC>,
    computation_reward: Balance<BFC>,
    wrapper: &mut SuiSystemState,
    new_epoch: u64,
...
public fun validator_stable_staking_pool_id<STABLE>(
    wrapper: &mut SuiSystemState,
    validator_addr: address,
): ID {
    let self = load_system_state(wrapper);
...
public fun validator_stable_staking_pool_mappings(
    wrapper: &mut SuiSystemState,
): &Table<ID, address> {
    let self = load_system_state(wrapper);
