...
use sui_system::stable_pool::{Self, StakedStable, PoolStableTokenExchangeRate};
use sui_system::stake_subsidy::StakeSubsidy;
use sui_system::staking_pool::{StakedBfc, FungibleStakedSui, PoolTokenExchangeRate};
...
public(package) fun request_add_validator(self: &mut SuiSystemStateInnerV2, ctx: &mut TxContext) {
    assert!(
...
public(package) fun advance_epoch(
    ...
    let safe_mode_computation_rewards = balance::withdraw_all(
        &mut self.safe_mode_computation_rewards,
    );
    ...
    let refunded_storage_rebate = storage_fund::advance_epoch(
...
public(package) fun validator_stake_amount_with_stable(
    ...
    validator_set::validator_total_stake_amount_with_stable(
        &self.validators,
        validator_addr,
        stable_rate,
    )
...
public(package) fun validator_stable_pool_id<STABLE>(
    self: &SuiSystemStateInnerV2,
    validator_addr: address,
): ID { validator_set::validator_stable_pool_id<STABLE>(&self.validators, validator_addr) }
...
public(package) fun validator_stable_staking_pool_mappings(
    self: &SuiSystemStateInnerV2,
): &Table<ID, address> { validator_set::stalbe_staking_pool_mappings(&self.validators) }
