...
module sui_system::validator_tests {
    ...
    fun test_pending_validator_flow() {
        ...
        test_scenario::next_tx(scenario, sender);
        {
            let coin_ids = test_scenario::ids_for_sender<StakedBfc>(scenario);
            let stake = test_scenario::take_from_sender_by_id<StakedBfc>(scenario, *vector::borrow(&coin_ids, 0));
            let ctx = test_scenario::ctx(scenario);
