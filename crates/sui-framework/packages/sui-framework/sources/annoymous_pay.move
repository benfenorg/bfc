...
module sui::annoymous_pay;

use sui::annoymous_coin::Annoymous_Coin;
...
public fun keep<T>(c: Annoymous_Coin<T>, ctx: &TxContext) {
    transfer::public_transfer(c, ctx.sender())
}

/// Split coin `self` to two coins, one with balance `split_amount`,
/// and the remaining balance is left is `self`.
public entry fun split<T>(coin: &mut Annoymous_Coin<T>, split_amount: u64, ctx: &mut TxContext) {
...
public entry fun split_vec<T>(self: &mut Annoymous_Coin<T>, split_amounts: vector<u64>, ctx: &mut TxContext) {
...
public entry fun divide_and_keep<T>(self: &mut Annoymous_Coin<T>, n: u64, ctx: &mut TxContext) {
    let mut vec: vector<Annoymous_Coin<T>> = self.divide_into_n(n, ctx);
...
public entry fun join<T>(self: &mut Annoymous_Coin<T>, coin: Annoymous_Coin<T>) {
    self.join(coin)
}

/// Join everything in `coins` with `self`
public entry fun join_vec<T>(self: &mut Annoymous_Coin<T>, mut coins: vector<Annoymous_Coin<T>>) {
...
public entry fun join_vec_and_transfer<T>(mut coins: vector<Annoymous_Coin<T>>, receiver: address) {
