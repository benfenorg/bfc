...
module sui::pay_tests {
    ...
    public entry fun test_coin_split_n() {
        ...
        let coin = coin::mint_for_testing<BFC>(10, ctx);

        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        pay::divide_and_keep(&mut coin, 3, test_scenario::ctx(scenario));

        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        let coin1 = test_scenario::take_from_sender<Coin<BFC>>(scenario);

        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        let coin2 = test_scenario::take_from_sender<Coin<BFC>>(scenario);
        ...
        assert!(
            !test_scenario::has_most_recent_for_sender<Coin<BFC>>(scenario),
            1
        );
    ...
    public entry fun test_split_and_transfer_fail() {
        ...
        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        let coin_transfer_fail = test_scenario::take_from_sender<Coin<BFC>>(scenario);
