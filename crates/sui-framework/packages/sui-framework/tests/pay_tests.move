...
fun test_coin_split_n() {
    ...
    let mut coin = coin::mint_for_testing<BFC>(10, ctx);

    scenario.next_tx(TEST_SENDER_ADDR);
    coin.divide_and_keep(3, scenario.ctx());

    scenario.next_tx(TEST_SENDER_ADDR);
    let coin1 = scenario.take_from_sender<Coin<BFC>>();

    scenario.next_tx(TEST_SENDER_ADDR);
    let coin2 = scenario.take_from_sender<Coin<BFC>>();

    scenario.next_tx(TEST_SENDER_ADDR);
    assert!(coin1.value() == 3);
    assert!(coin2.value() == 3);
    assert!(coin.value() == 4);
    assert!(!scenario.has_most_recent_for_sender<Coin<BFC>>(), 1);
...
fun test_split_and_transfer_fail() {
    ...
    scenario.next_tx(TEST_SENDER_ADDR);
    let coin_transfer_fail = scenario.take_from_sender<Coin<BFC>>();
