...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone)]
// `TokenDepositedEventV3` emitted in bridge.move
pub struct MoveTokenDepositedEventV3 {
    pub seq_num: u64,
    pub source_chain: u8,
    pub sender_address: Vec<u8>,
    pub target_chain: u8,
    pub target_address: Vec<u8>,
    pub token_type: u64,
    pub origin_token_type: u64,
    pub amount_before_fee: u64,
    pub amount_after_fee: u64,
}

#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone)]
// `TokenDepositedEventForSolanaV2` emitted in bridge.move
pub struct MoveTokenDepositedEventForSolanaV2 {
    pub seq_num: u64,
    pub source_chain: u8,
    pub sender_address: Vec<u8>,
    pub target_chain: u8,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone)]
// `TokenDepositedEventForSolanaV3` emitted in bridge.move
pub struct MoveTokenDepositedEventForSolanaV3 {
    pub seq_num: u64,
    pub source_chain: u8,
    pub sender_address: Vec<u8>,
    pub target_chain: u8,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone)]
pub struct MoveDefiTransferOutEvent {
    pub seq_num: u64,
    pub source_chain: u8,
    pub sender_address: Vec<u8>,
    pub target_chain: u8,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone)]
pub struct MoveDefiTokensStakedEvent {
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone)]
pub struct MoveTokenSendBackEventV2 {
    pub seq_num: u64,
    pub source_chain: u8,
    pub sender_address: Vec<u8>,
    pub target_chain: u8,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone, Hash)]
pub struct EmittedSuiToSolanaTokenBridgeV2 {
    pub nonce: u64,
    pub sui_chain_id: BridgeChainId,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone, Hash)]
pub struct EmittedSuiToEthDefiTokensStaked {
    pub nonce: u64,
    pub original_seq_num: u64,
    pub sui_chain_id: BridgeChainId,
    pub sui_address: SuiAddress,
...
impl TryFrom<MoveTokenDepositedEventV3> for EmittedSuiToEthTokenBridgeV1 {
    type Error = BridgeError;

    fn try_from(event: MoveTokenDepositedEventV3) -> BridgeResult<Self> {
        if event.amount_after_fee == 0 {
            return Err(BridgeError::ZeroValueBridgeTransfer(format!(
                "Failed to convert MoveTokenDepositedEventV3 to EmittedSuiToEthTokenBridgeV1. Manual intervention is required. 0 value transfer should not be allowed in Move: {:?}",
                event,
            )));
        }
        ...
        let sui_chain_id = BridgeChainId::try_from(event.source_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventV3 to EmittedSuiToEthTokenBridgeV1. Failed to convert source chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        let eth_chain_id = BridgeChainId::try_from(event.target_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventV3 to EmittedSuiToEthTokenBridgeV1. Failed to convert target chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        if !sui_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventV3 to EmittedSuiToEthTokenBridgeV1. Invalid source chain {}",
                event.source_chain
            )));
        }
        if eth_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventV3 to EmittedSuiToEthTokenBridgeV1. Invalid target chain {}",
                event.target_chain
            )));
        }

        let sui_address = SuiAddress::from_bytes(event.sender_address)
            .map_err(|e| BridgeError::Generic(format!("Failed to convert MoveTokenDepositedEventV3 to EmittedSuiToEthTokenBridgeV1. Failed to convert sender_address to SuiAddress: {:?}", e)))?;
        let eth_address = EthAddress::from_str(&Hex::encode(&event.target_address))?;
...
impl TryFrom<MoveTokenDepositedEventForSolanaV2> for EmittedSuiToSolanaTokenBridgeV2 {
    type Error = BridgeError;

    fn try_from(event: MoveTokenDepositedEventForSolanaV2) -> BridgeResult<Self> {
        if event.amount_after_fee == 0 {
            return Err(BridgeError::ZeroValueBridgeTransfer(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV2 to EmittedSuiToSolanaTokenBridgeV2. Manual intervention is required. 0 value transfer should not be allowed in Move: {:?}",
                event,
            )));
        }
        ...
        let sui_chain_id = BridgeChainId::try_from(event.source_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV2 to EmittedSuiToSolanaTokenBridgeV2. Failed to convert source chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        let solana_chain_id = BridgeChainId::try_from(event.target_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV2 to EmittedSuiToSolanaTokenBridgeV2. Failed to convert target chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        if !sui_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV2 to EmittedSuiToSolanaTokenBridgeV2. Invalid source chain {}",
                event.source_chain
            )));
        }
        if !solana_chain_id.is_solana_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV2 to EmittedSuiToSolanaTokenBridgeV2. Invalid target chain {}",
                event.target_chain
            )));
        }
        ...
        let sui_address = SuiAddress::from_bytes(event.sender_address).map_err(|e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV2 to EmittedSuiToSolanaTokenBridgeV2. Failed to convert sender_address to SuiAddress: {:?}",
                e
            ))
        })?;
        let solana_address_bytes: [u8; 32] = event
            ...
            .map_err(|_e| {
                BridgeError::Generic(format!(
                    "Failed to convert MoveTokenDepositedEventForSolanaV2 to EmittedSuiToSolanaTokenBridgeV2. Invalid solana target_address length: {}",
...
impl TryFrom<MoveTokenDepositedEventForSolanaV3> for EmittedSuiToSolanaTokenBridgeV2 {
    type Error = BridgeError;

    fn try_from(event: MoveTokenDepositedEventForSolanaV3) -> BridgeResult<Self> {
        if event.amount_after_fee == 0 {
            return Err(BridgeError::ZeroValueBridgeTransfer(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV3 to EmittedSuiToSolanaTokenBridgeV2. Manual intervention is required. 0 value transfer should not be allowed in Move: {:?}",
                event,
            )));
        }
        ...
        let sui_chain_id = BridgeChainId::try_from(event.source_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV3 to EmittedSuiToSolanaTokenBridgeV2. Failed to convert source chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        let solana_chain_id = BridgeChainId::try_from(event.target_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV3 to EmittedSuiToSolanaTokenBridgeV2. Failed to convert target chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        if !sui_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV3 to EmittedSuiToSolanaTokenBridgeV2. Invalid source chain {}",
                event.source_chain
            )));
        }
        if !solana_chain_id.is_solana_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV3 to EmittedSuiToSolanaTokenBridgeV2. Invalid target chain {}",
                event.target_chain
            )));
        }
        ...
        let sui_address = SuiAddress::from_bytes(event.sender_address).map_err(|e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEventForSolanaV3 to EmittedSuiToSolanaTokenBridgeV2. Failed to convert sender_address to SuiAddress: {:?}",
                e
            ))
        })?;
        let solana_address_bytes: [u8; 32] = event
            ...
            .map_err(|_e| {
                BridgeError::Generic(format!(
                    "Failed to convert MoveTokenDepositedEventForSolanaV3 to EmittedSuiToSolanaTokenBridgeV2. Invalid solana target_address length: {}",
                    event.target_address.len()
                ))
            })?;
        let solana_address = Pubkey::new_from_array(solana_address_bytes);
...
impl TryFrom<MoveDefiTransferOutEvent> for EmittedSuiToEthDefiBridgeV1 {
    type Error = BridgeError;
...
impl TryFrom<MoveDefiTokensStakedEvent> for EmittedSuiToEthDefiTokensStaked {
    type Error = BridgeError;

    fn try_from(event: MoveDefiTokensStakedEvent) -> BridgeResult<Self> {
        let sui_chain_id = BridgeChainId::try_from(event.source_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveDefiTokensStakedEvent to EmittedSuiToEthDefiTokensStaked. Failed to convert source chain {} to BridgeChainId",
                event.source_chain,
            ))
        })?;
        let eth_chain_id = BridgeChainId::try_from(event.target_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveDefiTokensStakedEvent to EmittedSuiToEthDefiTokensStaked. Failed to convert target chain {} to BridgeChainId",
                event.target_chain,
            ))
        })?;
        if !sui_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveDefiTokensStakedEvent to EmittedSuiToEthDefiTokensStaked. Invalid source chain {}",
                event.source_chain
            )));
        }
        if eth_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveDefiTokensStakedEvent to EmittedSuiToEthDefiTokensStaked. Invalid target chain {}",
                event.target_chain
            )));
        }

        let sui_address = SuiAddress::from_bytes(&event.sender_address)
            .map_err(|e| BridgeError::Generic(format!("Failed to convert MoveDefiTokensStakedEvent to EmittedSuiToEthDefiTokensStaked. Failed to convert sender_address to SuiAddress: {:?}", e)))?;

        Ok(Self {
            nonce: event.seq_num,
            original_seq_num: event.original_seq_num,
            sui_chain_id,
            sui_address,
            eth_chain_id,
            protocol_type: event.protocol_type,
            protocol_version: event.protocol_version,
            protocol_token_id: event.protocol_token_id,
            lp_token_amount: event.lp_token_amount,
            tx_hash: vec![],
            event_idx: 0,
        })
    }
...
impl TryFrom<MoveTokenSendBackEventV2> for EmittedEthTokenSendBackBridgeV1 {
    type Error = BridgeError;

    fn try_from(event: MoveTokenSendBackEventV2) -> BridgeResult<Self> {
        if event.amount_sui_adjusted == 0 {
            return Err(BridgeError::ZeroValueBridgeTransfer(format!(
                "Failed to convert MoveTokenSendBackEventV2 to EmittedEthTokenSendBackBridgeV1. Manual intervention is required. 0 value transfer should not be allowed in Move: {:?}",
                event,
            )));
        }
        ...
        let sui_chain_id = BridgeChainId::try_from(event.source_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenSendBackEventV2 to EmittedEthTokenSendBackBridgeV1. Failed to convert source chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        let eth_chain_id = BridgeChainId::try_from(event.target_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenSendBackEventV2 to EmittedEthTokenSendBackBridgeV1. Failed to convert target chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        if !sui_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenSendBackEventV2 to EmittedEthTokenSendBackBridgeV1. Invalid source chain {}",
                event.source_chain
            )));
        }
        if eth_chain_id.is_sui_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenSendBackEventV2 to EmittedEthTokenSendBackBridgeV1. Invalid target chain {}",
                event.target_chain
            )));
        }

        let sui_address = SuiAddress::from_bytes(event.sender_address)
            .map_err(|e| BridgeError::Generic(format!("Failed to convert MoveTokenSendBackEventV2 to EmittedEthTokenSendBackBridgeV1. Failed to convert sender_address to SuiAddress: {:?}", e)))?;
        let eth_address = EthAddress::from_str(&Hex::encode(&event.target_address))?;
...
crate::declare_events!(
    ...
    SuiToEthTokenBridgeV2(EmittedSuiToEthTokenBridgeV1) => ("bridge::TokenDepositedEventV2", MoveTokenDepositedEventV2),
    SuiToEthTokenBridgeV3(EmittedSuiToEthTokenBridgeV1) => ("bridge::TokenDepositedEventV3", MoveTokenDepositedEventV3),
    SuiToSolanaTokenBridgeV2(EmittedSuiToSolanaTokenBridgeV2) => ("bridge::TokenDepositedEventForSolanaV2", MoveTokenDepositedEventForSolanaV2),
    SuiToSolanaTokenBridgeV3(EmittedSuiToSolanaTokenBridgeV2) => ("bridge::TokenDepositedEventForSolanaV3", MoveTokenDepositedEventForSolanaV3),
    SuiToEthDefiBridgeV1(EmittedSuiToEthDefiBridgeV1) => ("bridge::DefiTransferOutEvent", MoveDefiTransferOutEvent),
    SuiToEthDefiTokensStaked(EmittedSuiToEthDefiTokensStaked) => ("bridge::DefiTokensStakedEvent", MoveDefiTokensStakedEvent),
...
impl SuiBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        match self {
            ...
            SuiBridgeEvent::SuiToEthTokenBridgeV2(event) => {
                Some(BridgeAction::SuiToEthBridgeAction(SuiToEthBridgeAction {
                    sui_tx_digest,
                    sui_tx_event_index,
                    sui_bridge_event: event.clone(),
            ...
            SuiBridgeEvent::SuiToEthTokenBridgeV3(event) => {
                Some(BridgeAction::SuiToEthBridgeAction(SuiToEthBridgeAction {
                    sui_tx_digest,
                    sui_tx_event_index,
                    sui_bridge_event: event.clone(),
            ...
            SuiBridgeEvent::SuiToSolanaTokenBridgeV2(event) => {
                Some(BridgeAction::SuiToSolanaBridgeAction(SuiToSolanaBridgeAction {
                    sui_tx_digest,
                    sui_tx_event_index,
                    sui_bridge_event: event.clone(),
                }))
            }
            SuiBridgeEvent::SuiToSolanaTokenBridgeV3(event) => {
                Some(BridgeAction::SuiToSolanaBridgeAction(SuiToSolanaBridgeAction {
                    sui_tx_digest,
                    sui_tx_event_index,
                    sui_bridge_event: event.clone(),
            ...
            SuiBridgeEvent::SuiToEthDefiBridgeV1(event) => {
                Some(BridgeAction::SuiToEthDefiBridgeAction(SuiToEthDefiBridgeAction {
                    sui_tx_digest,
                    sui_tx_event_index,
                    sui_bridge_event: event.clone(),
                }))
            }
            SuiBridgeEvent::SuiToEthDefiTokensStaked(_event) => {
                None
            }
...
pub mod tests {
    ...
    pub fn get_test_sui_event_and_action(identifier: Identifier) -> (SuiEvent, BridgeAction) {
     ...
     /// Returns a test SuiEvent and corresponding BridgeAction
     pub fn get_test_sui_event_v2_and_action(identifier: Identifier) -> (SuiEvent, BridgeAction) {
        init_all_struct_tags(); // Ensure all tags are initialized
        let sanitized_event = EmittedSuiToEthTokenBridgeV1 {
            nonce: 1,
            sui_chain_id: BridgeChainId::SuiTestnet,
        ...
        let emitted_event = MoveTokenDepositedEventV2 {
            seq_num: sanitized_event.nonce,
            source_chain: sanitized_event.sui_chain_id as u8,
            sender_address: sanitized_event.sui_address.to_vec(),
            target_chain: sanitized_event.eth_chain_id as u8,
            ...
            token_type: sanitized_event.token_id,
            amount_before_fee: 0,
            amount_after_fee: sanitized_event.amount_sui_adjusted,
        };
        ...
        let event = SuiEvent {
            type_: SuiToEthTokenBridgeV2.get().unwrap().clone(),
            bcs: BcsEvent::new(bcs::to_bytes(&emitted_event).unwrap()),
            id: EventID {
                tx_digest,
                event_seq: event_idx as u64,
    ...
    #[test]
    fn test_0_sui_amount_conversion_for_sui_event_v2() {
        let emitted_event = MoveTokenDepositedEventV2 {
            seq_num: 1,
            source_chain: BridgeChainId::SuiTestnet as u8,
            sender_address: SuiAddress::random_for_testing_only().to_vec(),
