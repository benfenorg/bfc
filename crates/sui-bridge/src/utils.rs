...
/// Airdrop SOL（使用 poll_for_signature，最稳定）
pub async fn airdrop_sol(client: &Arc<RpcClient>, pubkey: &Pubkey, sol: u64) -> anyhow::Result<()> {
    let sig = client.request_airdrop(pubkey, sol * LAMPORTS_PER_SOL)?;
...
/// 部署 USDC
pub async fn deploy_usdc(
    client: &Arc<RpcClient>,
    payer: &Keypair,
    mint_amount: u64,
    decimals: u8,
) -> anyhow::Result<USDCDeploymentResult> {
    airdrop_sol(client, &payer.pubkey(), 2).await?;
    println!("Payer balance: {}", client.get_balance(&payer.pubkey())?);
