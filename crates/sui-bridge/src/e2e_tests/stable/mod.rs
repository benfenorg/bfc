...
#[allow(dead_code)]
pub async fn mint_stable_coin_to_address(amount: u64, http_client: &HttpClient, address: SuiAddress, key_pair: &SuiKeyPair, coint_type: &str,to_address: SuiAddress) -> Result<(), Error> {
    println!("mint_stable_coin_to_address address: {:?}", address);
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        ...
        .await?;
    let tx_data = transaction_bytes.to_data()?;
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::sui_transaction(), &tx_data),
        key_pair,
    );
    let (tx_bytes, signatures) = Transaction::from_data(tx_data, vec![signature]).to_tx_bytes_and_signatures();
