...
pub struct BridgeOrchestratorTables {
    ...
    /// pending actions that are waiting for fast path
    pub(crate) pending_fast_path_actions: DBMap<BridgeActionDigest, BridgeAction>,
}
...
impl BridgeOrchestratorTables {
    ...
    pub(crate) fn insert_pending_fast_path_actions(
        &self,
        actions: &[BridgeAction],
    ) -> BridgeResult<()> {
        let mut batch = self.pending_fast_path_actions.batch();
        batch
            .insert_batch(
                &self.pending_fast_path_actions,
                actions.iter().map(|a| (a.digest(), a)),
            )
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Couldn't insert into pending_fast_path_checked_actions: {:?}",
                    e
                ))
            })?;
        batch
    ...
    pub(crate) fn remove_fast_path_actions(
        &self,
        actions: &[BridgeActionDigest],
    ) -> BridgeResult<()> {
        let mut batch = self.pending_fast_path_actions.batch();
        batch
            .delete_batch(&self.pending_fast_path_actions, actions)
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Couldn't delete from pending_fast_path_actions: {:?}",
                    e
                ))
            })?;
        batch
    ...
    pub fn get_all_pending_actions_4_fast_path(&self) -> HashMap<BridgeActionDigest, BridgeAction> {
        self.pending_fast_path_actions.unbounded_iter().filter(
            |(_, action)| {
                info!("[DEBUG]  get_all_pending_actions_4_fast_path: {:#?}", action);
                true
            },
        ).collect()
    }
