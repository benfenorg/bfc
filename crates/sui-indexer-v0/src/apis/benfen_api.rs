...
use sui_json_rpc::api::{
    validate_limit, BenfenApiServer, QUERY_MAX_RESULT_LIMIT, QUERY_MAX_RESULT_LIMIT_CHECKPOINTS,
};
...
pub(crate) struct BenfenApi<S> {
    state: S,
    fullnode: HttpClient,
    config: IndexerConfig,
}

impl<S: IndexerStore> BenfenApi<S> {
...
#[async_trait]
impl<S: IndexerStore + Sync + Send + 'static> BenfenApiServer for BenfenApi<S> {
...
impl<S> SuiRpcModule for BenfenApi<S>
where
    S: IndexerStore + Sync + Send + 'static,
{
    ...
    fn rpc_doc_module() -> Module {
        sui_json_rpc::api::BenfenApiOpenRpc::module_doc()
    }
}
