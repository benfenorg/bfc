...
bfc client --client.config $CONFIG publish "dependency" --verify-deps \
  --json | jq '.effects.status'

echo "=== publish package v0 (should NOT warn) ===" | tee /dev/stderr
UPGRADE_CAP=$(bfc client --client.config $CONFIG publish "example" --verify-deps \
...
bfc client --client.config $CONFIG upgrade --upgrade-capability $UPGRADE_CAP example --verify-deps \
  --json | jq '.effects.status'
...
bfc client --client.config $CONFIG publish "example" --verify-deps \
