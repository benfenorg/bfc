...
module examples::anonymous_coin;

use sui::anonymous_coin::{Self, TreasuryCap};

// The type identifier of coin. The coin will have a type
// tag of kind: `Coin<package_object::mycoin::MYCOIN>`
// Make sure that the name of the type matches the module's name.
public struct ANONYMOUS_COIN has drop {}

// Module initializer is called once on module publish. A treasury
// cap is sent to the publisher, who then controls minting and burning.
fun init(witness: ANONYMOUS_COIN, ctx: &mut TxContext) {
...
public fun mint(
    treasury_cap: &mut TreasuryCap<ANONYMOUS_COIN>,
    amount: u64,
    recipient: address,
    ctx: &mut TxContext,
) {
    let coin = anonymous_coin::mint(treasury_cap, amount, ctx);
