...
use crate::events::MinSingleTransferLimitUpdated;


#[derive(Accounts)]
pub struct UpdateMinSingleTransferLimit<'info> {
    ...
    #[account(
        ...
        seeds = [
            ...
            &[message::UPDATE_BRIDGE_MIN_SINGLE_TRANSFER_LIMIT],
            verifier.key().as_ref()
        ],
        bump
    )]
...
pub fn update_min_single_transfer_limit_with_signatures(
 ...
 let mut message_config =  ctx.accounts.message_config.deref_mut();
    ...
    require!(message_type==message::UPDATE_BRIDGE_MIN_SINGLE_TRANSFER_LIMIT,MessageError::InvalidMessageType);
    ...
    let (source_chain_id, new_limit)=message::decode_update_min_single_transfer_limit_payload(&payload)?;
    require!(chain_limit.get_chain_id()==source_chain_id, BridgeLimiterError::InvalidChainId);
    chain_limit.set_min_single_transfer_limit(new_limit);

    msg!("emit MinSingleTransferLimitUpdated");

    emit!(MinSingleTransferLimitUpdated {
        nonce,
        source_chain_id,
        new_limit,
    });
