...
pub fn transfer_sol_from_user_to_bridge_vault<'info>(
    ...
    // Get the minimum balance required for rent exemption (0-byte account)
    let rent_exempt_minimum = rent.minimum_balance(0);
    
    // Ensure user has sufficient balance for transfer while maintaining rent-exempt minimum balance
    require!(
        user_lamports >= amount.saturating_add(rent_exempt_minimum),
        BridgeError::InsufficientBalance
    );

    // Transfer SOL from user to bridge vault
...
/// Transfer SOL from bridge vault to user
...
/// Transfer SPL tokens from user to bridge vault
