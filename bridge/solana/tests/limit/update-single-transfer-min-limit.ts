...
describe("BenfenBridge - Update Single Transfer Min Limit", () => {
    ...
    it("update single transfer min limit",async()=>{

        let message_type=MESSAGE_TYPES.UPDATE_BRIDGE_MIN_SINGLE_TRANSFER_LIMIT;
        let nonce = 0; 
        let version=1;
        let chain_id=CHAIN_IDS.SOLANA_TESTNET;

        let limit=1*100000000; //1 USD
        ...
        expect(signatures.length).to.be.equal(3);
         ...
         let tx=(await program.methods.updateMinSingleTransferLimit(
            message_type,
            version,
            new BN(nonce),
            chain_id,
        ...
        expect(chainLimit.minUsdLimit.toNumber()).to.be.equal(limit);

        let messageConfig1 = await program.account.messageConfig.fetch(messageConfigPDA);
        expect(messageConfig1.messageType).to.be.equal(MESSAGE_TYPES.UPDATE_BRIDGE_MIN_SINGLE_TRANSFER_LIMIT);
