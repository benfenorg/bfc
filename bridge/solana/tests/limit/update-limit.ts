...
import { createMessage,createAddSlpTokenPayload, computeMessageHash,createUpdateLimitPayload } from "../../utils/message";
...
describe("BenfenBridge - Update Limit", () => {
    ...
    it("update limit with invalid message type",async ()=>{
        ...
        const [messageConfigPDA]= anchor.web3.PublicKey.findProgramAddressSync(
            [Buffer.from(SEEDS.MESSAGE_CONFIG),Buffer.from([MESSAGE_TYPES.UPDATE_BRIDGE_LIMIT]),messageVerifierPDA.toBuffer()],
            program.programId
        );
      
        let updateLimitPayload= createUpdateLimitPayload(
            targetChainId,
            BigInt(limit),
        );
        ...
        try {
            await program.methods.updateBridgeLimiter(
                message_type,
                version,
                new BN(nonce),
                chain_id,
                Buffer.from(updateLimitPayload),
            ...
            expect.fail("Expected updateBridgeLimiter to fail with invalid message type");
        }catch (error) {
            console.log(error)
            expect(error.message).to.include("Invalid message type");

        }
    })    

    it("update limit",async()=>{
        let message_type=MESSAGE_TYPES.UPDATE_BRIDGE_LIMIT;
        let nonce = 0; 
        let version=1;
        let chain_id=CHAIN_IDS.SOLANA_TESTNET;
        let limit=500*100000000;
        ...
        expect(signatures.length).to.be.equal(3);
         ...
         let tx=(await program.methods.updateBridgeLimiter(
            message_type,
            version,
            new BN(nonce),
            chain_id,
