...
If you are building a wallet, we publish a helper library `@benfen/wallet-standard` which provides
...
`@benfen/wallet-standard` to help ensure your class adheres to the standard.

```tsx
import { Wallet, SUI_DEVNET_CHAIN } from '@benfen/wallet-standard';
...
```tsx
import { ReadonlyWalletAccount } from '@benfen/wallet-standard';
