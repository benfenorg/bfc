...
- If your function `f` needs a payment in (e.g.) SUI from the caller, use `fun f(payment: Coin<BFC>)` not `fun f(payment: &mut Coin<BFC>, amount: u64)`. This is safer for callers—they know exactly how much they are paying, and do not need to trust `f` to extract the right amount.
