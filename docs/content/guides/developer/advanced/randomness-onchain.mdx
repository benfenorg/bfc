...
A similar attack to the one previously described involves PTBs *even* when `play_dice` is defined as a private `entry` function. For example, consider the `entry play_dice(guess: u8, fee: Coin<BFC>, r: &Random, ctx: &mut TxContext): Option<GuessedCorrectly> { … }` function defined earlier, the attacker can publish the function
...
```move
entry fun play(r: &Random, payment: Coin<BFC>, ...) {
  ...
  let generator = new_generator(r, ctx);
