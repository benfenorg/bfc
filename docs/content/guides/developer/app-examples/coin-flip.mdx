...
```move title='house_data.move'
    public fun initialize_house_data(house_cap: HouseCap, coin: Coin<BFC>, public_key: vector<u8>, ctx: &mut TxContext) {
...
```move title='single_player_satoshi.move'
    ...
    fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<BFC>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
        map_guess(guess);
