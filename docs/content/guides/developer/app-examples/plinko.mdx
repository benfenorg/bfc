...
public fun start_game(counter: &mut Counter, num_balls: u64, coin: Coin<BFC>, house_data: &mut HouseData, ctx: &mut TxContext): ID {
...
public fun finish_game(game_id: ID, bls_sig: vector<u8>, house_data: &mut HouseData, num_balls: u64, ctx: &mut TxContext): (u64, address, vector<u8>) {
    ...
    let stake_per_ball = balance::value<BFC>(&stake) / num_balls;
...
fun internal_start_game(counter: &mut Counter, num_balls: u64, coin: Coin<BFC>, house_data: &HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
    ...
    let new_game = Game {
        ...
        stake: coin::into_balance<BFC>(coin),
...
public fun initialize_house_data(house_cap: HouseCap, coin: Coin<BFC>, public_key: vector<u8>, multiplier: vector<u64>, ctx: &mut TxContext) {
...
public fun top_up(house_data: &mut HouseData, coin: Coin<BFC>, _: &mut TxContext) {
...
public(friend) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<BFC> {
    &mut house_data.balance
}

/// Returns a mutable reference to the fees of the house.
public(friend) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<BFC> {
    &mut house_data.fees
}
