module polynet::tools {
    ...
    public  fun init_as_mainnet(
        ...
        init_mainnet_ccm(_config,_ctx);
        issue_license_to_lock_proxy(_config, _ctx);
    }

    public entry fun init_mainnet_ccm(_global: &mut CrossChainGlobalConfig, _ctx: &mut TxContext) {
        // sender address
        let sender = tx_context::sender(_ctx);
        assert!(utils::is_admin(sender), EINVALID_ADMIN);
        ...
        cross_chain_manager::update_cross_chain_config(cc_manager, keepers, startHeight, polyId, _ctx);
    }
    ...
    public  fun init_as_testnet(
        ...
        init_testnet_ccm(_config,_ctx);
        issue_license_to_lock_proxy(_config, _ctx);
    }



    public entry fun init_testnet_ccm(_global: &mut CrossChainGlobalConfig,_ctx: &mut TxContext) {
        // sender address
        let sender = tx_context::sender(_ctx);
        assert!(utils::is_admin(sender), EINVALID_ADMIN);
