#[allow(unused_variable,unused_assignment,unused_function,unused_field)]
module tokens::cross_chain_utils {
    use std::hash;
    use std::vector;
    //use std::option;
    //use tokens::secp256k1;
    //use tokens::utils as putil;
    //use sui::ecdsa_k1;
    ...
    public fun merkleProve(auditPath: &vector<u8>, root: &vector<u8>): vector<u8> {
        let offset: u64 = 0;
        let value: vector<u8>;
        (value, offset) = tokens::zero_copy_source::next_var_bytes(auditPath, offset);
        ...
        let pos: u8;
        while (index < size) {
            (pos, offset) = tokens::zero_copy_source::next_byte(auditPath, offset);
            (nodeHash, offset) = tokens::zero_copy_source::next_hash(auditPath, offset);
            hash = if (pos == 0) {
    ...
    public fun verifySig(rawHeader: &vector<u8>,
                         sigList: &vector<u8>,
        ...
        vector<u8>) 
    {
        ...
        (txHash, offset) = tokens::zero_copy_source::next_var_bytes(valueBs, offset);
        (fromChainID, offset) = tokens::zero_copy_source::next_u64(valueBs, offset);

        (txParam_txHash, offset) = tokens::zero_copy_source::next_var_bytes(valueBs, offset);
        (txParam_crossChainId, offset) = tokens::zero_copy_source::next_var_bytes(valueBs, offset);
        (txParam_fromContract, offset) = tokens::zero_copy_source::next_var_bytes(valueBs, offset);
        (txParam_toChainId, offset) = tokens::zero_copy_source::next_u64(valueBs, offset);
        (txParam_toContract, offset) = tokens::zero_copy_source::next_var_bytes(valueBs, offset);
        (txParam_method, offset) = tokens::zero_copy_source::next_var_bytes(valueBs, offset);
        (txParam_args, _) = tokens::zero_copy_source::next_var_bytes(valueBs, offset);
        ...
        vector<u8>)
    {
        ...
        (version, offset) = tokens::zero_copy_source::next_u32(headerBs , offset);
        (chainId, offset) = tokens::zero_copy_source::next_u64(headerBs , offset);
        (prevBlockHash, offset) = tokens::zero_copy_source::next_hash(headerBs , offset);
        (transactionsRoot, offset) = tokens::zero_copy_source::next_hash(headerBs , offset);
        (crossStatesRoot, offset) = tokens::zero_copy_source::next_hash(headerBs , offset);
        (blockRoot, offset) = tokens::zero_copy_source::next_hash(headerBs , offset);
        (timestamp, offset) = tokens::zero_copy_source::next_u32(headerBs , offset);
        (height, offset) = tokens::zero_copy_source::next_u32(headerBs , offset);
        (consensusData, offset) = tokens::zero_copy_source::next_u64(headerBs , offset);
        (consensusPayload, offset) = tokens::zero_copy_source::next_var_bytes(headerBs , offset);
        (nextBookkeeper, _) = tokens::zero_copy_source::next_bytes20(headerBs , offset);
