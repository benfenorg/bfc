...
module capy::capy_winter {
    ...
    public struct CapyPost has key { id: UID, balance: Balance<BFC> }
    ...
    // public struct PremiumTicketUsed has copy, drop { id: ID }
    ...
    // public struct OpenedKey has store, copy, drop { sender: address }
    ...
    fun merge_and_split(
        mut coins: vector<Coin<BFC>>, amount: u64, ctx: &mut TxContext
    ): (Coin<BFC>, Coin<BFC>) {
