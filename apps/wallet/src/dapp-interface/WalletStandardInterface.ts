...
export class SuiWallet implements Wallet {
	...
	#signTransaction: BenfenSignTransactionMethod = async ({ transaction, account, chain }) => {
		return mapToPromise(
	...
	#signTransactionBlock: BenfenSignTransactionBlockMethod = async ({
		...
		return mapToPromise(
			...
			(response) => ({
				transactionBlockBytes: response.result.bytes,
				signature: response.result.signature,
			}),
		);
	};

	#signAndExecuteTransaction: BenfenSignAndExecuteTransactionMethod = async (input) => {
		return mapToPromise(
